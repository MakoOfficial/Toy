...
class Multi_Sacle_Fusion(nn.Module):
    ...
    def forward(self, feature1, feature2, feature3, feature4):
        fusion_feature1 = feature1 + self.fusion1_upsample1(feature2) + self.fusion1_upsample2(feature3) + self.fusion1_upsample3(feature4)

        fusion_feature2 = self.fusion2_DW(feature1) + feature2 + self.fusion2_upsample1(feature3) + self.fusion2_upsample2(feature4)

        fusion_feature3 = self.fusion3_DW1(feature1) + self.fusion3_DW2(feature2) + feature3 + self.fusion3_upsample(feature4)

        fusion_feature4 = self.fusion4_DW1(feature1) + self.fusion4_DW2(feature2) + self.fusion4_DW3(feature3) + feature4
...
class Toy(nn.Module):
    ...
    def __init__(self, genderSize, backbone, out_channels) -> None:
        ...
        self.MMCA1 = MMCA_module(512)
        self.backbone2 = backbone[5]
        self.MMCA2 = MMCA_module(1024, reduction=[8, 8], level=2)
        self.backbone3 = backbone[6]
        self.MMCA3 = MMCA_module(2048, reduction=[8, 16], level=2)
        self.backbone4 = backbone[7]
        self.MMCA4 = MMCA_module(4096, reduction=[4, 8, 8], level=3)

        self.K_means1 = KMEANS(n_clusters=19)
        self.K_means2 = KMEANS(n_clusters=19)
        self.K_means3 = KMEANS(n_clusters=19)
        self.K_means4 = KMEANS(n_clusters=19)
        ...
        self.MLP4 = nn.Sequential(
            nn.Linear(4096 + genderSize, 2048),
            nn.BatchNorm1d(2048),
            nn.ReLU(),
            nn.Linear(2048, 1024),
            nn.BatchNorm1d(1024),
    ...
    def Prototype_Initialization(self, image):
        self.train(False)
        batch_size = image.shape[0]

        P1 = self.backbone1(image)
        self.K_means1.fit(P1.detach())
        self.prototype1 = self.K_means1.centers

        P2 = self.backbone2(P1)
        self.K_means2.fit(P2.detach())
        self.prototype2 = self.K_means2.centers

        P3 = self.backbone3(P2)
        self.K_means3.fit(P3.detach())
        self.prototype3 = self.K_means3.centers

        P4 = self.backbone4(P3)
        self.K_means4.fit(P4.detach())
        self.prototype4 = self.K_means4.centers

        # return self.feature1[1:], self.feature2[1:], self.feature3[1:], self.feature4[1:]
    ...
    def forward(self, image, gender):
        ...
        feature_for_reg1 = torch.cat([torch.squeeze(F.adaptive_avg_pool2d(self.MMCA1(feature_for_attention1), 1)), gender_encode], dim=1)
        feature_for_attention2 = torch.cat([fusion_feature2, fusion_res2], dim=1)
        feature_for_reg2 = torch.cat([torch.squeeze(F.adaptive_avg_pool2d(self.MMCA2(feature_for_attention2), 1)), gender_encode], dim=1)
        feature_for_attention3 = torch.cat([fusion_feature3, fusion_res3], dim=1)
        feature_for_reg3 = torch.cat([torch.squeeze(F.adaptive_avg_pool2d(self.MMCA3(feature_for_attention3), 1)), gender_encode], dim=1)
        feature_for_attention4 = torch.cat([fusion_feature4, fusion_res4], dim=1)
        feature_for_reg4 = torch.cat([torch.squeeze(F.adaptive_avg_pool2d(self.MMCA4(feature_for_attention4), 1)), gender_encode], dim=1)
